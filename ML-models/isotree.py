...
logger = logging.getLogger("extended_isolation_forest")
...
def train(annotations_file,
    ...
    logger.info(f'Hyperparameters: {hyperparameters_kwargs}')

    # Add a print statement here to output hyperparameters right before training
    print("Hyperparameters being used for training:", hyperparameters_kwargs)
    ...
    Extended_iso_forest = IsolationForest(**hyperparameters_kwargs)
    logger.info(f'Isolation Forest model initialized with parameters: {Extended_iso_forest.get_params()}')  # New logging statement to confirm model parameters

    Extended_iso_forest.fit(imputed_X_ni)
    ...
    logger.info('Training Extend Isolation Forest...')
    Extended_iso_forest = IsolationForest(**hyperparameters_kwargs)
    Extended_iso_forest.fit(imputed_X_ni)
    logger.info('Training complete.')
    
    # Properly indented score_samples function
    #def score_samples(test_annotation_names_i, test_X_ni):
    ...
    #    return Extended_iso_forest.predict(imputer.transform(test_X_ni), output='score') 
    def score_samples(test_annotation_names_i, test_X_ni):
        assert np.array_equal(test_annotation_names_i, annotation_names_i), \
            f'Input annotation names ({test_annotation_names_i}) must be identical to those used to train the scorer ({annotation_names_i}).'
        original_scores = Extended_iso_forest.predict(imputer.transform(test_X_ni), output='score')
        # Inverting the scores
        inverted_scores = 1 - original_scores
        return inverted_scores

    # Properly indented scorer_lambda
    scorer_lambda = lambda test_annotation_names_i, test_X_ni: score_samples(test_annotation_names_i, test_X_ni)

    # Properly indented logging and pickling code
